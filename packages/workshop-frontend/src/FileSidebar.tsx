...
export default function FileSidebar({
  ...
  const handleCreateFile = () => {
    if (!newFileName.trim()) {
      toasts.add({
        title: formatMessage({ id: 'fileSidebar.toastFilenameEmpty', defaultMessage: 'Filename cannot be empty' }),
        variant: 'error',
      })
      return
    }
    ...
    if (files.includes(newFileName.trim())) {
      toasts.add({
        title: formatMessage({
          id: 'fileSidebar.toastFileExists', defaultMessage: 'A file with this name already exists',
        }),
        variant: 'error',
      })
      return
  ...
  const startDelete = (filename: string) => {
    if (files.length <= 1) {
      toasts.add({
        title: formatMessage({
          id: 'fileSidebar.toastCannotDeleteLast', defaultMessage: 'Cannot delete the last remaining file',
        }),
        variant: 'error',
      })
      return
    ...
    <div className="flex h-full w-[244px] flex-col border-r border-kumo-line bg-kumo-base">
      <div className="flex h-9 shrink-0 items-center justify-between gap-2 px-3 pt-3 pb-2">
        <span className="text-[11px] font-medium uppercase tracking-[0.08em] text-kumo-inactive">
          <FormattedMessage id="fileSidebar.filesHeading" defaultMessage="Files" />
        </span>
        <WorkshopIconButton
          onClick={() => setIsCreateModalOpen(true)}
          disabled={editLocked}
          aria-label={formatMessage({ id: 'fileSidebar.newFile', defaultMessage: 'New file' })}
          title={formatMessage({ id: 'fileSidebar.newFile', defaultMessage: 'New file' })}
        ...
        >
          <div className="flex items-start justify-between gap-4 border-b border-kumo-line px-5 py-4">
            <div className="min-w-0">
              <Dialog.Title className="text-[15px] leading-5 font-medium tracking-[-0.3px] text-kumo-default">
                <FormattedMessage id="fileSidebar.newFileDialogTitle" defaultMessage="New file" />
              </Dialog.Title>
              <Dialog.Description className="mt-1 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                <FormattedMessage id="fileSidebar.newFileDialogDescription" defaultMessage="Create a new file in this gadget." />
              </Dialog.Description>
            </div>
            <Dialog.Close
              render={(props) => (
                <WorkshopIconButton
                  {...props}
                  className="!h-7 !w-7"
                  aria-label={formatMessage({ id: 'fileSidebar.close', defaultMessage: 'Close' })}
                >
                  <X size={16} />
                </WorkshopIconButton>
              )}
          ...
          <div className="px-5 py-4">
            <WorkshopInput
              ...
              aria-label={formatMessage({ id: 'fileSidebar.filenameAriaLabel', defaultMessage: 'Filename' })}
              value={newFileName}
          ...
          <div className="flex items-center justify-end gap-2 border-t border-kumo-line bg-kumo-base px-5 py-3">
            <Dialog.Close
              render={(props) => (
                <WorkshopButton
                  ...
                  className="!h-9"
                >
                  <FormattedMessage id="fileSidebar.cancel" defaultMessage="Cancel" />
                </WorkshopButton>
              )}
            />
            <WorkshopButton
            ...
            >
              <FormattedMessage id="fileSidebar.createFile" defaultMessage="Create file" />
            </WorkshopButton>
          </div>
        </Dialog>
      </Dialog.Root>
      ...
      <DeleteConfirmationDialog
        ...
        title={formatMessage({ id: 'fileSidebar.deleteFileTitle', defaultMessage: 'Delete file?' })}
        description={(
          <FormattedMessage
            id="fileSidebar.deleteFileDescription"
            defaultMessage="This removes {filename} from the gadget. You can't undo this."
            values={{ filename: <span className="font-mono text-kumo-default">{deletingFile}</span> }}
          />
        )}
      ...
      {isRenaming ? (
        <input
          ...
          aria-label={formatMessage(
            { id: 'fileSidebar.renameFilename', defaultMessage: 'Rename {filename}' },
            { filename },
          )}
        ...
        >
          ...
          {isStreamingActive && (
            <span
              ...
              aria-label={formatMessage(
                { id: 'fileSidebar.filenameBeingEdited', defaultMessage: '{filename} is being edited' },
                { filename },
              )}
              title={formatMessage({
                id: 'fileSidebar.agentEditingTitle', defaultMessage: 'Agent is editing this file',
              })}
            />
          )}
        </button>
        ...
        <DropdownMenu>
          <DropdownMenu.Trigger
            render={(
              <WorkshopIconButton
                aria-label={formatMessage(
                  { id: 'fileSidebar.actionsForFilename', defaultMessage: 'Actions for {filename}' },
                  { filename },
                )}
                onClick={(event) => event.stopPropagation()}
          ...
          >
            ...
            >
              <FormattedMessage id="fileSidebar.download" defaultMessage="Download" />
            </DropdownMenu.Item>
            {!editLocked && (
              <>
                <DropdownMenu.Item
                  ...
                  className="!h-auto rounded-md !px-2.5 !py-1.5 text-[12px] leading-4 tracking-[-0.2px] text-kumo-default transition-colors data-highlighted:bg-kumo-tint"
                >
                  <FormattedMessage id="fileSidebar.rename" defaultMessage="Rename" />
                </DropdownMenu.Item>
                <DropdownMenu.Item
                ...
                >
                  <FormattedMessage id="fileSidebar.delete" defaultMessage="Delete" />
                </DropdownMenu.Item>
              </>
            )}
          </DropdownMenu.Content>
